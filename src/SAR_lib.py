"""
PROYECTO COORDINADO SAR-ALT (RECUPERADOR)
...
class SAR_Project:
    ...
    def __init__(self):
        ...
        self.speller = spellsuggester.SpellSuggester()
        self.use_spelling = False
        self.distance = None
        self.threshold = None
        self.set_spelling(False, None, None)
    ...
    def index_dir(self, root, **args):
        ...
        for field in self.index:
            for term in self.index[field]:
                voc.append(term)
        self.speller.set_vocabulary(sorted(set(voc)))
        ...
        if('*' in term) or ('?' in term):
            ...
            # ALG
                ...
                if self.use_spelling:
                    suggestions = self.speller.suggest(term, self.distance, self.threshold)
                    for sugg in suggestions:
                        result = self.or_posting(result, self.get_posting(sugg))
